...
app.post("/api/v1/mailboxes/:mailboxId/emails", async (c: AppContext) => {
	...
	c.executionCtx.waitUntil(
		sendEmail(c.env.EMAIL, {
			...
			new Logger({ component: "sendEmail", mailboxId }).error("Deferred email delivery failed", {
				messageId,
				to,
...
async function receiveEmail(event: { raw: ReadableStream; rawSize: number }, env: Env, ctx: ExecutionContext) {
	...
	if (allowedAddresses.length > 0) {
		...
		if (!mailboxId) {
		new Logger({ component: "receiveEmail" }).info("Ignoring email: no recipient matches EMAIL_ADDRESSES", {
	...
	if (!(await env.BUCKET.head(`mailboxes/${mailboxId}.json`))) {
		new Logger({ component: "receiveEmail" }).info("Ignoring email: mailbox does not exist", {
			mailboxId,
	...
	ctx.waitUntil(agentStub.fetch(new Request("https://agents/onNewEmail", {
		...
		const error = e instanceof Error ? e : new Error(String(e));
		new Logger({ component: "autoDraft", mailboxId }).error("Auto-draft trigger failed", {
			emailId: messageId,
